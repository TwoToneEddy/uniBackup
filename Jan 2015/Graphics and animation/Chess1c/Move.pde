...
boolean MakeAMove (String Move1) {
  ...
  if (valid) {
    if (showFiguresAsImages) {
      // if 2D images (and not Bauhaus figures)
      // execute immediately 
      moveRunning=false;
      ...
      moveRunningX_ScreenAdd = (moveRunningX_ScreenTo-moveRunningX_ScreenFrom)/50 ; 
      moveRunningY_ScreenAdd = (moveRunningY_ScreenTo-moveRunningY_ScreenFrom)/50 ;
